...
export async function getFormats(): Promise<ApiResult<FormatsResponse>> {
  ...
  try {
    const res = await fetch(API_URLS.getAudioFormats(), {  // 发送 GET 请求
      method: 'GET',
      headers: { accept: 'application/json' },  // 告诉后端期望 JSON 响应
    });
    if (res.ok) {  // HTTP 2xx → 解析 JSON 并返回
...
export async function convertAudio(
  ...
  try {
    const res = await fetch(API_URLS.convertAudio(), {  // 发送 POST 请求
      method: 'POST',
...
export function filterSupportedFiles(
  ...
  for (const f of files) {
    ...
    if (supportedExtensions.length > 0 && !supportedExtensions.includes(ext)) {  // 白名单检查
      rejected.push({ name: f.name, reason: `不支持的格式 .${ext}` });
      continue;  // 跳过此文件
    }
    if (f.size > MAX_FILE_SIZE) {  // 大小上限检查
