  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        ...(showFitFields &&
          maxWidth > 0 &&
          maxHeight > 0 && { max_width: maxWidth, max_height: maxHeight }),
        ...(showExactFields &&
          exactWidth > 0 &&
          exactHeight > 0 && { width: exactWidth, height: exactHeight }),
      ...
      <div className={styles.topRow}>
        ...
        >
          {files.length === 0 ? (
            <EmptyState
              title="拖拽图片到此处，或点击选择"
              description={`支持 ${supportedExtensions.join(' / ')}，单文件 ≤100MB`}
            />
          ) : (
        ...
        <div className={styles.panel}>
          ...
          {targetFormat === 'ico' && files.length > 0 && (
            <div className={styles.paramRow}>
              <span className={styles.hintText}>非方形图片将居中裁切为正方形后再转换为 ICO</span>
            </div>
          )}
