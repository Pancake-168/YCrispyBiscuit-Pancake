...
const getStartupLogFilePath = () => path.join(app.getPath('userData'), 'startup.log')
...
function initializeDataPath() {
  try {
    ...
    console.log('[System:main.ts:initializeDataPath] UserData redirected to:', userDataPath)
  } catch (err) {
...
// 必须在开头尽早调用
initializeDataPath()
logStartupTiming('initializeDataPath completed', `(userData=${app.getPath('userData')})`)
logStartupTiming('startup context', `(packaged=${app.isPackaged} execPath=${process.execPath} baseDir=${getAppBaseDir()})`)
...
const getStoreConfigFilePath = () => path.join(app.getPath('userData'), 'config.json')
const getStoreBackupFilePath = () => path.join(app.getPath('userData'), `config.plaintext.backup.${Date.now()}.json`)
const getStoreCorruptedBackupFilePath = () => path.join(app.getPath('userData'), `config.unrecoverable.backup.${Date.now()}.json`)
const getLegacyStoreKeyFilePath = () => path.join(app.getPath('userData'), 'config.key')
...
const createEncryptedStoreInstance = (userDataPath: string, encryptionKey: string) => {
  return new Store({ cwd: userDataPath, encryptionKey }) as unknown as StoreInstance
}

const readPlaintextStoreSnapshot = (userDataPath: string) => {
  const plainStore = new Store({ cwd: userDataPath }) as unknown as StoreInstance
  return { ...plainStore.store }
}

const writeEncryptedStoreSnapshot = (userDataPath: string, encryptionKey: string, snapshot: Record<string, unknown>) => {
  const encryptedStore = createEncryptedStoreInstance(userDataPath, encryptionKey)
...
const removeLegacyStoreKeyFile = () => {
  ...
  try {
    ...
    console.warn('[System:main.ts:removeLegacyStoreKeyFile] 删除旧密钥文件失败:', error)
  }
}
...
const tryReadPlaintextStoreSnapshot = (userDataPath: string) => {
  try {
    ...
    console.warn('[System:main.ts:tryReadPlaintextStoreSnapshot] 明文配置读取失败:', error)
    return null
  }
}
...
const resetCorruptedStoreFiles = () => {
  ...
  if (fs.existsSync(configFilePath)) {
    try {
      ...
      console.warn('[System:main.ts:resetCorruptedStoreFiles] 已备份无法恢复的配置文件:', corruptedBackupPath)
    } catch (error) {
      console.warn('[System:main.ts:resetCorruptedStoreFiles] 备份损坏配置失败，尝试直接删除:', error)
      try {
        fs.unlinkSync(configFilePath)
      } catch (deleteError) {
        console.warn('[System:main.ts:resetCorruptedStoreFiles] 删除损坏配置失败:', deleteError)
      }
    }
  }
...
const getStore = (): StoreInstance => {
    ...
    logStartupTiming('getStore start')
        ...
        try {
          ...
          console.warn('[System:main.ts:getStore] 读取加密配置失败，尝试迁移或重建:', error)
          ...
          if (fs.existsSync(configFilePath)) {
            ...
            if (plaintextSnapshot) {
              ...
              console.log('[System:main.ts:getStore] 已备份明文配置并迁移为加密配置:', backupPath)
              store = writeEncryptedStoreSnapshot(userDataPath, loadStoreEncryptionKey(), plaintextSnapshot)
            } else {
...
const createWindow = () => {
  ...
  if (!app.isPackaged) {
    logStartupTiming('createWindow loadURL(dev) start', `(url=${devServerUrl})`)
    win.loadURL(devServerUrl)
  } else {
    // 打包后使用自定义 app:// 协议承载前端，保证 History 路由刷新不 404
    logStartupTiming('createWindow loadURL(app) start', `(url=${APP_PROTOCOL}://-/)`)
