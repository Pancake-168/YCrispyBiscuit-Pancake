...
const apiMode =
  (import.meta.env.VITE_API_MODE as unknown as string | undefined) || "";
const useRelativeApi =
  import.meta.env.DEV ||
  apiMode === "proxy" ||
  (!isElectronRuntime && apiMode !== "direct");
export const BASE_URL = useRelativeApi
  ? ""
  : (import.meta.env.VITE_API_BASE as unknown as string);
...
export const API_URLS = {
  ...
  // 获取用户某个指定音乐列表中的音乐
  getUserMusicListMusics: (musicListId: string) =>
    `${BASE_URL}/api/pancake/user/musiclist/${musicListId}`,

  // getMusicDetail get
  // 获取音乐详情
  getMusicDetail: (musicId: string) =>
    `${BASE_URL}/api/pancake/music/${musicId}`,

  // getLyrics get
  // 获取歌词
  getLyrics: (musicId: string) =>
    `${BASE_URL}/api/pancake/music/${musicId}/lyrics`,
  ...
  getArtistDetail: (artistId: string) =>
    `${BASE_URL}/api/pancake/artist/${artistId}`,

  // getAlbumDetail get
  // 获取专辑详情
  getAlbumDetail: (albumId: string) =>
    `${BASE_URL}/api/pancake/album/${albumId}`,
};
