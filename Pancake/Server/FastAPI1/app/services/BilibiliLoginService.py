...
from app.services.BilibiliPageService import fetch_fingerprint, fetch_nav_info  # 指纹 + 导航信息
...
class BilibiliLoginService:
    ...
    def __init__(self, sessions: dict[str, BilibiliSession], qrcode_store: dict[str, dict]):
        self._sessions = sessions  # 共享的全局会话字典（key=session_id, value=BilibiliSession）
    ...
    async def get_login_url(self) -> dict:
        ...
        if data["code"] != 0:  # B站 API 返回非 0 表示失败
            raise ExternalServiceError(f"Bilibili QR generate failed: {data}")
        qr_data = data["data"]  # 提取 data 子对象
        key = qr_data["qrcode_key"]  # 二维码唯一标识
        qr_content = qr_data["url"]  # 二维码承载的 URL（B站登录页）
    ...
    async def poll_qr_login(self, qrcode_key: str) -> dict:
        ...
        # code == 0 → 用户已确认登录
        session = await self._build_session_from_qr(resp, data["data"])  # 构建会话
    ...
    async def login_by_cookie(self, cookie_str: str) -> str:
        ...
        # 手动解析 Cookie 字符串 "key1=val1; key2=val2"
        for item in cookie_str.replace(" ", "").split(";"):
            if "=" in item:
                k, v = item.split("=", 1)  # 仅按第一个 = 分割
                if k.strip():
                    session.cookies[k.strip()] = v.strip()
        session.client.cookies.update(session.cookies)  # 同步到 httpx 客户端
        # 拉取指纹和 nav 信息以验证 cookies 是否有效
    ...
    async def _build_session_from_qr(self, response, login_data: dict) -> BilibiliSession:
        """从扫码登录的 HTTP 响应和 login_data 构建 BilibiliSession。"""
        session = BilibiliSession()
        session.absorb_response_cookies(response)  # 从响应头提取 Set-Cookie
