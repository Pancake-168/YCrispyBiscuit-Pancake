...
class FormatDetail:
    ...
    - supports_animation:   是否支持动画/多帧    → 前端未使用此字段控制 UI
...
FORMAT_DETAILS: Dict[str, FormatDetail] = {
    ...
    # SVG 矢量化输出不支持透明通道（路径填充为纯色），故 supports_transparency=False
...
def detect_format_by_magic(data: bytes, filename: str = "") -> Optional[str]:
    ...
    if len(data) >= 12 and data[4:8] == b"ftyp":
        ...
        if brand in (b"avif", b"avis"):  # AVIF 的两种 brand
            return "AVIF"
        if brand in (b"heic", b"heix", b"hevc", b"hevx", b"mif1"):  # HEIF/HEIC 的各种 brand
            return "HEIF"
...
def _fit_into(orig_w: int, orig_h: int, bound_w: int, bound_h: int) -> Tuple[int, int]:
    ...
    - 如果 ratio >= 1.0（图片比边界小），不做放大，保持原尺寸
    - 否则按 ratio 等比缩小
    - 结果用 max(1, ...) 保证至少 1px
    """
    ratio = min(bound_w / orig_w, bound_h / orig_h)  # 取较小比例 → 保证不超出
    if ratio >= 1.0:
        return orig_w, orig_h  # 图片不大于边界，无需缩放
    return max(1, round(orig_w * ratio)), max(1, round(orig_h * ratio))  # 等比缩小
...
def resolve_color_mode(
    ...
    if target_format in no_alpha_formats:
        ...
        if img_mode in ("RGBA", "LA", "PA") or has_transparency:
            return "RGB"  # 原图有透明信息→丢弃 alpha，后续 _apply_color_mode 用背景色填充
...
def get_save_kwargs(
    ...
    quality: Optional[int] = None, # 质量参数（None=使用 Pillow 默认值；前端 quality 滑块的值）
    ...
    if fmt is None:
        fmt = target_format.upper()  # 最终 fallback：纯大写（防御 get_pillow_format 失败的极端情况）

    kwargs: Dict[str, Any] = {}  # 收集的保存参数，最终传给 img.save(buf, format=..., **kwargs)

    if fmt == "JPEG":
        if quality is not None:
            kwargs["quality"] = max(1, min(100, quality))  # JPEG 不接受 quality=0，下限钳位到 1
        kwargs["optimize"] = optimize  # Pillow 的 optimize 会对 Huffman 表做优化，减小体积
        ...
        if lossless:
            ...
            kwargs["quality"] = max(0, min(100, quality))  # WebP 接受 quality=0（极端压缩）
