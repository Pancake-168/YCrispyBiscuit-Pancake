...
class FormatDetail(BaseModel):
    ...
    quality_range: Optional[List[int]] = (
        None  # 质量范围 [min, max] 或 null → 决定滑块 min/max
    )
...
class FormatsResponse(BaseModel):
    ...
    format_details: dict[
        str, FormatDetail
    ]  # key=格式标识 value=FormatDetail，与 output_formats 对齐
...
class ConvertResultItem(BaseModel):
    ...
    warning: Optional[str] = (
        None  # 成功时的提示信息（如“多帧图片仅保留首帧”），无提示时为 null
    )
