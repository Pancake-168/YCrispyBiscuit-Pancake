...
class PictureTaskStore:
    ...
    def __init__(self, output_dir: Path, max_tasks: int, cleanup_seconds: int):
        ...
        self._tasks: OrderedDict[str, list[StoredFile]] = OrderedDict()  # 任务字典（按插入顺序）
    ...
    def register(self, task_id: str, task_dir: Path, stored: list[StoredFile]) -> None:
        ...
        if len(stored) > 1:
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
                for sf in stored:
                    zf.write(sf.path, sf.converted_name)  # 写入 zip（文件在 zip 内的名字为 converted_name）
    ...
    def get_filename(self, task_id: str, index: int) -> str:
        ...
        return stored[index].converted_name if index < len(stored) else "converted"  # 兜底
