...
设计原则（照单全收）：会话不预设任何字段清单，
B站返回什么 cookie 就存什么，导出时全量给出，不做字段筛选。
"""
...
class BilibiliSession:
    """单个 B 站登录会话：持有 cookies、token 和预配置的 httpx.AsyncClient。"""

    def __init__(self):
        # 当前会话的 cookies 字典（不预设键，来什么存什么）
    ...
    def absorb_response_cookies(self, resp: httpx.Response):
        """从 httpx 响应中提取所有 Set-Cookie 并合并到会话 cookies（全量吸收）。"""
