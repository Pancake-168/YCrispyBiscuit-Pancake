"""Bilibili 登录与信息获取 API 端点。

- GET    /api/bilibili/login/url               获取扫码登录二维码
- GET    /api/bilibili/login/poll              轮询扫码状态
- POST   /api/bilibili/login/cookie            Cookie 直接登录
- GET    /api/bilibili/user                    用户信息（nav 完整响应）
- GET    /api/bilibili/stored-values           全量存储值聚合
- GET    /api/bilibili/ac-time-value           ac_time_value 与页面变量
- GET    /api/bilibili/sessions                列出活跃会话
- DELETE /api/bilibili/session                 删除会话

本模块不抛任何业务异常：所有校验与错误都在 service 层完成。
所有响应体均为全量透传（照单全收），不筛选字段。
"""
...
from app.schemas.BilibiliSchema import (
    ...
    BilibiliStoredValuesResponse,  # 全量存储值响应
    BilibiliAcTimeValueResponse,  # ac_time_value 响应
)
from app.services.Bilibili.BilibiliService import (
    BilibiliService,
...
async def get_login_url(
    service: BilibiliService = Depends(get_bilibili_service),  # 注入全局单例服务
):
    """获取B站二维码登录的 URL 和 qrcode_key，前端据此生成二维码图片。"""
    result = await service.get_login_url()  # 调 B站 API 生成 qrcode_key
    return BilibiliLoginUrlResponse(**result)  # dict → Pydantic 全量透传
...
async def get_user_info(
    ...
    """根据 session_id 获取 nav 接口完整响应（含 mid、uname、vip 等，照单全收）。"""
    result = await service.get_user_info(session_id)  # 调 B站 nav 接口
...
async def get_stored_values(
    ...
    """获取当前会话中所有可用的 B站存储值（cookies/token/nav/指纹/页面变量全量聚合）。"""
...
async def get_ac_time_value(
    ...
    """获取 ac_time_value 与时间戳，附页面变量全量提取（真实值随接口响应透传）。"""
    result = await service.get_ac_time_value(session_id)  # 计算并全量返回
...
async def delete_session(
    ...
    return {"message": f"会话 {session_id} 已删除"}  # 返回确认消息
