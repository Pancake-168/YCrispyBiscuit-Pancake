...
from app.services.PCmethodsService import (
    PCmethodsService,
    get_pcmethods_service,
)  # PC方法服务 + 工厂
...
async def get_mmd_paths(
    ...
    workflow = (
        await service.get_mmd_workflow()
    )  # 从 PCmethods.json 中查找 MMD 工作流条目
...
@router.post(
    "/pcmethods/openmmd/{folder_name}",  # 路径: /api/pcmethods/openmmd/MMD
    summary="单独打开MMD工作流中指定名称的文件夹",
    tags=["PCmethods"],
    response_model=PCmethodsOpenResponse,
)
async def open_single_folder(
    folder_name: str,
    service: PCmethodsService = Depends(get_pcmethods_service),
):
    """在 Windows 资源管理器中打开 MMD 工作流中指定名称的单个文件夹。"""
    message = await service.open_single_mmd_folder(folder_name)
