...
class WeatherService:
    ...
    def fetch_weather_data(self,locatID:str) -> dict:
        """获取天气数据"""
        ...
        try:
            ...
            if parsed.query:
                ...
                if qdict:
                    ...
                    url_without_q = urlunparse((parsed.scheme, parsed.netloc, parsed.path, parsed.params, '', parsed.fragment))
                    resp = requests.get(url_without_q, params={key: locatID}, headers=headers, timeout=8)
                else:
                    request_url = base + str(locatID).lstrip('/')
                    resp = requests.get(request_url, headers=headers, timeout=8)
            else:
