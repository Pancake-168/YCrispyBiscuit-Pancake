<template>
  <div class="setpost-overlay" @click.self="handleClose">
    <div class="setpost-modal">
      ...
      <div class="modal-body">
        ...
        <div class="two-col">
          ...
          <div class="col left">
            <ApplicationUserSelectorV2
              :appid="props.appid"
              targetType="user"
              title="用户列表选择器"
              :inline="true"
              :confirm="false"
              :onSelect="(u: any) => addUser(Number(u.id))"
              :onClose="() => {}"
            />
          </div>
...
import { UpdateOrganizationPostV2 } from '@/services/Project/OrganizationV2/Post';
import ApplicationUserSelectorV2 from '@/components/NewOrganizationAndDepartmentV2/ApplicationUserSelectorV2/index.vue';
...
const appUsers = computed(() => applicationUsersByAppId.value.get(props.appid)?.data || []);

function displayName(uid: number) {
  const u = appUsers.value.find((x: any) => Number(x.id) === Number(uid)) as ApplicationUserItem | undefined;
...
function updateStorePostSnapshot(payload: { name: string; description: string; userIds: number[] }) {
  ...
  for (const u of appUsers.value as any[]) {
