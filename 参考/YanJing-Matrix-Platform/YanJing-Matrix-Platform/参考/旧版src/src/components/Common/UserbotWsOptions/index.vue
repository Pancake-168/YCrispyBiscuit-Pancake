...
import { userbotWebSocketService } from '@/services/userbot/websocket'
import type {
    ResolveUserForOptionSend,
    UserbotWsOptionSendPayload,
    UserbotWsOptionsRaw,
} from '@/services/userbot/options'
import {
...
interface Props {
     ...
     * 你也可以在接入时注入自定义 send（例如走别的通道/加埋点）。
     */
    send?: (payload: UserbotWsOptionSendPayload) => Promise<void> | void

    /**
     * user 构建钩子：你说入站消息可能没有 user，需要你自己构建。
...
const onSelect = async (option: string) => {
    ...
    try {
        ...
        console.log("[UserbotWsOptions] 发送选项 payload:", payload)
