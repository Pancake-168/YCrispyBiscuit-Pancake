<template>
    <div class="matrix-register-form">
        <form @submit.prevent="handleSubmit" class="register-form">
            <h1 class="login-title">微信绑定账号</h1>
            <div class="login-underline"></div>
            <p class="bind-hint">您的微信尚未关联账号，请设置一个新的手机号码进行绑定。</p>
            <div class="svg">
            ...
            <div class="form-group">
                ...
                <div class="input-icon-wrapper">
                    ...
                    <input id="reg-password" v-model="formData.password" :disabled="isRegistering" placeholder="请输入手机号"
...
// 本地定义绑定参数类型，移除 homeserver
interface WechatBindParams {
    username: string;
    password: string;
    confirmPassword: string;
}

const formData = reactive<WechatBindParams>({
    username: '',
    password: '',
    confirmPassword: ''
})
...
const isFormValid = computed(() => {
    ...
    return formData.password.length === 11 &&
...
const handleSubmit = async () => {
    if (!isFormValid.value) {
        errorMessage.value = '请确保填写有效的手机号'
        return
    }
    /*
...
const resetRegisterState = (message?: string) => {
