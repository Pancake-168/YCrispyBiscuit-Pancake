...
import { linkifyString, linkifyHtml, options as linkifyOptions } from '../Vue-Markdown/linkify-matrix'
...
const safeHtml = computed(() => {
   // console.log('原始内容:', props.content)
    ...
    try {
        // 预处理内容
        let content = preprocessContent(props.content)
...
const handleClick = (event: MouseEvent) => {
    ...
    const isNocoBaseLink = nocoBaseHosts.some(host => href.includes(host))
    ...
    nextTick(() => {
        if (props.streamMode) {
            ...
            try {
                ...
                streamHtml.value = md.toHTML({ externalLinks: false })
            } catch (e) {
                console.error('Initial stream render error:', e)
            }
        } else {
            addCopyButtons()
        ...
        if (newLength > oldLength) {
            ...
            nextTick(() => {
                try {
                    ...
                    console.error('Stream render error:', e)
                }
            })
        }
    } else {
...
<style scoped>
/* ============================================
   Element 官方 Markdown 样式
   完全照搬自 Element Web 源代码
...
.markdown-body {
    ...
    letter-spacing: var(--cpd-font-letter-spacing-body-md, 0);
...
.markdown-body :deep(.mx_Pill:hover),
