...
function matrixOpaqueIdLinkifyParser({
  ...
  const matrixTokenState = new linkifyjs.State(matrixToken) as any

  const matrixTokenWithPort = linkifyjs.createTokenClass(name, { isLink: true })
  const matrixTokenWithPortState = new linkifyjs.State(matrixTokenWithPort) as any
...
// 注册 Matrix 房间别名插件
registerPlugin(Type.RoomAlias, ({ scanner, parser }: any) => {
...
// 注册 Matrix 用户ID插件
registerPlugin(Type.UserId, ({ scanner, parser }: any) => {
