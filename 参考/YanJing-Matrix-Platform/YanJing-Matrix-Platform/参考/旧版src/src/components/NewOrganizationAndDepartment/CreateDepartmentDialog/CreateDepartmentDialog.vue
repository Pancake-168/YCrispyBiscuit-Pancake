    ...
    <div class="dialog" @click.stop>
      ...
      <div class="dialog-body">
        <div class="form-section">
          <label>部门名称</label>
          <input v-model="departmentName" placeholder="请输入部门名称" @keyup.enter="createDepartment" :disabled="creating" />
...
import { createDepartment as createDepartmentAPI } from '@/services/Project/Organization/data/data'
...
const validateName = () => {
  ...
  if (name.length < 2) {
    nameError.value = '部门名称至少需要2个字符'
    return false
  }
  if (name.length > 50) {
    nameError.value = '部门名称不能超过50个字符'
    return false
  }
  nameError.value = ''
  return true
...
const createDepartment = async () => {
  ...
  console.log('[CreateDepartmentDialog] 开始创建部门', {
  ...
  try {
    ...
    console.log('[CreateDepartmentDialog]  部门创建成功')
    emit('created', departmentName.value.trim())

  } catch (error: any) {
    const errorMessage = error.message || '创建部门时发生未知错误'
    result.value = {
      success: false,
      error: errorMessage
    }
    console.error('[CreateDepartmentDialog]  部门创建失败:', error)
  } finally {
