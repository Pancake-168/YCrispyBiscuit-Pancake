...
const handleSearchInput = () => {
  ...
  searchTimeout.value = setTimeout(async () => {
    ...
    try {
      ...
      searchResults.value = await 搜索用户(query, { limit: 10, query })
    } catch (error) {
...
const selectUser = (user: UserSearchResult) => {
  ...
  userDetailsMap.value.set(user.userId, {
    displayName: getDisplayName(user.userId, user.displayName),
...
const sendInvites = async () => {
  ...
  if (props.inviteType === 'space') {
    try {
      ...
      console.error("[InviteDialog] 获取空间成员失败，将尝试直接邀请", e);
    }
  }
  ...
  try {
    ...
    for (let i = 0; i < inviteList.value.length; i++) {
      ...
      // 判断是否需要跳过 Matrix 邀请（比如已经在空间内）
      let skipMatrixInvite = false
      // 空间邀请时，如果已经在空间内，则跳过 Matrix 协议层的邀请，但不能跳过 NocoBase 的业务同步
      if (props.inviteType === 'space') {
        if (memberIds.includes(userId)) {
          console.log(`[InviteDialog] 用户 ${userId} 已在空间内，跳过 Matrix 邀请，继续执行业务同步`);
          skipMatrixInvite = true;
        }
      }

      try {
        console.log(`[InviteDialog] 正在处理第 ${i + 1}/${inviteList.value.length} 个用户: ${userId}`)


        // 1. 调用 Matrix 邀请
        // 只有两种情况执行：
        // A. 普通房间邀请 (inviteType !== 'space')
        // B. 空间邀请且用户不在空间内 (!skipMatrixInvite)
        if (props.inviteType !== 'space' || !skipMatrixInvite) {
        ...
        // 2. 调用业务侧同步 (NocoBase) - 仅限空间邀请
        if (props.inviteType === 'space') {
          try {
            await InviteUserToApplication(props.roomId, userId)
          } catch (e: any) {
            // 如果是因为用户已经在房间内导致的报错，忽略之，继续执行后续的部门加入逻辑
            const errorMsg = e.message || '';
            if (errorMsg.includes('is already in the room') || errorMsg.includes('M_FORBIDDEN')) {
              console.log(`[InviteDialog] 业务侧邀请提示用户已在房间内，忽略错误继续执行: ${userId}`, errorMsg);
            } else {
              // 其他错误则抛出
              throw e;
            }
