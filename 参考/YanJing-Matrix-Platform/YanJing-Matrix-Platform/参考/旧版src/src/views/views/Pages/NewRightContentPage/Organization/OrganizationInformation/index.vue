...
const { currentOrganization, orgTrees, flatDeptMaps } = storeToRefs(store);

// 获取当前组织的树结构
const currentOrgTree = computed(() => {
  if (!currentOrganization.value?.application_id) return [];
  return orgTrees.value.get(currentOrganization.value.application_id) || [];
});

// 获取根节点（通常是组织节点）
const rootNode = computed(() => currentOrgTree.value[0] || null);

// 组织名称
const orgName = computed(() => currentOrganization.value?.application_name || '未选择组织');

// 管理员名单
const adminNames = computed(() => {
  if (!rootNode.value?.children) return '未指定';
  // 只查找组织直属的管理员
  const admins = rootNode.value.children
    .filter(child => child.type === 'person' && (child.role === '主管' || child.role === '管理员'))
    .map(admin => admin.nickname || admin.name);
  return admins.length > 0 ? admins.join('、') : '未指定';
});
...
const orgInfoList = computed(() => {
  ...
  if (appId) {
    const map = flatDeptMaps.value.get(appId);
    if (map) {
      // 遍历扁平 Map 统计所有部门类型的节点
  ...
  return [
    { label: '地区', value: '未指定' }, // 目前 API 未返回此字段
    { label: '行业', value: '未指定' }, // 目前 API 未返回此字段
    { 
      label: '部门数量', 
...
const handleQuit = async () => {
  const confirmed = await openConfirmDialog('确认退出？', {
    title: '确认退出',
    confirmText: '退出',
    cancelText: '取消',
  })
  if (!confirmed) return
  // TODO: 实际退出逻辑
  console.log('退出组织:', currentOrganization.value?.application_name);
};
</script>
