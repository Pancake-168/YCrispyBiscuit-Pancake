<template>
	<div class="system-list">
		<section class="panel panel--mission-summary" v-if="mission">
			<header class="panel__header">
				<div class="panel__title">
					<button class="back-button" type="button" @click="handleBackToMissionList">
						←
					</button>
					<div class="title-group">
						<span class="title">{{ mission.title }}</span>
						<span class="meta" v-if="mission.lastUpdated">更新于 {{ mission.lastUpdated }}</span>
					</div>
				</div>
			</header>
			<div class="panel__body">
				<p class="mission-summary" v-if="roomTopic">{{ roomTopic }}</p>
				<p class="mission-summary mission-summary--placeholder" v-else>暂无任务主题。</p>
			</div>
		</section>
    ...
    <section class="panel panel--resources" v-if="mission">
			<header class="panel__header">
				<div class="panel__title">
...
const { currentMission } = storeToRefs(assistantSidebarStore)

const chatContext = inject('chatContext') as any

const mission = computed(() => currentMission.value)

// 获取当前房间主题
const roomTopic = computed(() => {
  const roomId = chatContext?.currentRoomId?.value
  if (!roomId) return ''
...
// 文件列表（基于当前房间）
const roomIdRef = computed(() => chatContext?.currentRoomId?.value || null)
...
const handleBackToMissionList = () => {
  assistantSidebarStore.backToMissionList()
}
</script>
