                        ...
                        <div class="main-content">
                            <div class="Docus-page">
                                ...
                                <div class="app-info-card">
                                    ...
                                    <n-button v-if="source !== 'my-apps'" @click="showCopyAppModal = true"
                                    ...
                                    <div v-else style="color: #999; font-size: 0.9rem; margin-top: 12px;">
                                        * 您正在浏览智能体市场，仅可查看
                                    </div>
                                </div>
...
import { NCard, NButton, NForm, NFormItem, NInput, NModal, useMessage } from 'naive-ui';
import { publicAgent,handleMarketCopyApplication, editorAgent, destroyAgent, publicApplication, fetchPublicApplications, fetchUserApplications, handleCreateApplication, fetchApplicationAgents, createAgent } from '../Data/Application_Data';
import type { ApiApplicationItem } from '../Data/Application_Data';
import Markdown_For_Market from '../../Markdown_For_Market';
...
async function getAppInfo() {
    // 获取公有应用
    const publicRes = await fetchPublicApplications();
    // 获取个人应用
    const userRes = await fetchUserApplications(defaultUserId);
    ...
    // 查找目标应用
    const targetApp = allApps.find(app => String(app.application_id) === String(applicationId)) || null;
    appInfo.value = targetApp;
    console.log('应用信息:', appInfo.value);
    // 判断是否有 agentTeams
    if (targetApp && Array.isArray(targetApp.agentTeams) && targetApp.agentTeams.length > 0) {
        agentList.value = targetApp.agentTeams;
        console.log('使用 agentTeams 作为 agentList:', agentList.value);
    } else if (targetApp) {
        // 没有 agentTeams，调用接口获取
        try {
            const { ok, data } = await fetchApplicationAgents(String(appInfo.value?.id ?? ''));
            if (ok && Array.isArray(data)) {
                agentList.value = data;
                console.log('使用 fetchApplicationAgents 作为 agentList:', agentList.value);
            } else {
                agentList.value = [];
            }
        } catch (e) {
            agentList.value = [];
            console.error('fetchApplicationAgents异常:', e);
        }
    } else {
        agentList.value = [];
    }
...
async function setAppPublic() {
    ...
    try {
        ...
        // 调用API，无论是设为公开还是私有
        const res = await publicApplication(appInfo.value.id.toString(), !currentIsPublic);
        ...
        if (res.ok) {
            ...
            // 更新本地状态
            appInfo.value.isPublic = !currentIsPublic ? "true" : "false";
...
async function deleteAgent() {
    ...
    try {
        ...
        const res = await destroyAgent({ agent_id: currentAgent.value.id.toString() });
...
async function handleCreate() {
    ...
    creating.value = true;
    try {
        const { ok, data } = await handleCreateApplication({
...
async function handleeditorAgent(appid: string, agentAccount: string, userAccount: string) {
    ...
    try {
        const res = await editorAgent({ appid, agentAccount, userAccount });
        if (res.ok) {
        } else {
...
async function handleCopyApp() {
    ...
    try {
        const res = await handleMarketCopyApplication({
