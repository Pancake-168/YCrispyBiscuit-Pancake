...
// 动态查找当前房间内的 Bot（以 IDMap 为准：matrixId + nickname + type）
const activeRoomBot = computed<ActiveRoomBot | null>(() => {
  const roomId = injectedCurrentRoomId?.value || props.messages[0]?.roomId
  ...
  try {
    ...
    // 仅考虑 join / invite 的成员（与 useUserbotWebSocket 的策略保持一致）
    const activeMembers = members.filter(
      (m: any) => !m?.membership || m.membership === 'join' || m.membership === 'invite'
    )

    const bots: ActiveRoomBot[] = []
    for (const m of activeMembers) {
      ...
      if (!mapped || mapped.type !== 'bot') continue
      ...
      bots.push({
        matrixId: mapped.matrixId,
        username: mapped.username,
        nickname: mapped.nickname,
        joinTs: Number.isFinite(joinTs) ? joinTs : 0,
      })
    }
    ...
    console.error('[MessageArea] 查找房间 Bot 失败:', e)
    return null
  }
})
...
const displayMessages = computed(() => {
  ...
  if (streamMessage && (streamMessage.content || streamMessage.thinkContent)) {
    console.log('[MessageArea] 添加流式消息到显示列表', {
      content: streamMessage.content,
      thinkContent: streamMessage.thinkContent
    })
    
    // 使用动态查找到的 Bot 信息（以 IDMap 为准）；找不到则回退到临时占位身份
    const bot = activeRoomBot.value || {
      matrixId: `@streaming-bot:${currentRoomId}`,
      username: '',
      nickname: 'AI助手',
      joinTs: 0,
    }
    ...
    realMessages.push({
      ...
      messageType: 'm.text',
      isStreaming: true,
      displayName: bot.nickname || bot.username || 'AI助手',
      encrypted: false,
...
const TOP_THRESHOLD = 80 // px，接近顶部即触发

const getActiveRoomId = () => injectedCurrentRoomId?.value || displayMessages.value[0]?.roomId || ''
...
const mergeRoomMessages = (roomId: string, fresh: MatrixMessage[]) => {
  try {
    ...
    console.warn('[MessageArea] 合并历史消息失败:', e)
  }
}
...
const loadMoreIfNeeded = async () => {
  ...
  try {
    ...
    console.warn('[MessageArea] 加载更多历史失败:', e)
  } finally {
...
const handleScroll = () => {
  if (!messagesContainer.value) return
  userHasScrolled.value = !isUserAtBottom()
  // 顶部触发加载
  loadMoreIfNeeded()
}
...
watch(() => props.messages, (newMessages, oldMessages) => {
  ...
  if (newMessages.length > (oldMessages?.length || 0)) {
    ...
    const roomId = lastMsg.roomId || injectedCurrentRoomId?.value || newMessages[0]?.roomId || ''
    const stream = roomId ? streamMessages.value.get(roomId) : undefined
    const streamTs = stream?.timestamp || 0

    // 容忍客户端/服务器时间差，避免历史回填或时间漂移导致误删/不删
    const STREAM_CLEANUP_SKEW_MS = 30_000
    const isNewEnough = streamTs ? (lastMsg.timestamp >= streamTs - STREAM_CLEANUP_SKEW_MS) : true
    
    // 动态判断 Sender 是否是 Bot
    let isBotMessage = false
    const bot = activeRoomBot.value

    if (bot && lastMsg.sender === bot.matrixId) {
      isBotMessage = true
    } else if (!bot && lastMsg.sender !== props.currentUserId) {
      // Fallback：仅当确实存在流式占位时，才允许“非我消息”结束流式（避免多人房间误删）
      if (stream && isNewEnough) {
        isBotMessage = true
      }
    }

    // 如果最新消息是 bot 的回复,清理对应房间的流式消息
    if (isBotMessage && isNewEnough) {
...
// wire up read receipts composable
// read receipts 不应包含流式占位消息（eventId 非真实 Matrix 事件，且 timestamp 会持续更新）
const messagesRef = computed(() => displayMessages.value.filter(m => !m.isStreaming))
const roomIdRef = computed(() => injectedCurrentRoomId?.value || displayMessages.value[0]?.roomId || '')
...
// 当房间切换后，如果内容仍不足以滚动，再次尝试补齐
watch(roomIdRef, async () => {
  await nextTick()
  void autoFillUntilScrollable()
})
...
const autoFillUntilScrollable = async (maxBatches: number = 10) => {
  ...
  while (loops < maxBatches) {
    ...
    try {
      ...
      console.warn('[MessageArea/Bot] 自动补齐历史失败:', e)
      break
    } finally {
      isLoadingMore.value = false
      loops++
...
@media (max-width: 768px) {
  ...
  .message-group :deep(.left-message-content),
  .message-group :deep(.right-message-content) {
    min-width: 0;
  }

  .message-group :deep(.left-message-header),
  .message-group :deep(.right-message-header) {
    max-width: calc(100vw - 72px);
    min-width: 0;
  }

  .message-group :deep(.left-message-body),
  .message-group :deep(.right-message-body) {
    max-width: calc(100vw - 72px);
    min-width: 0;
    width: fit-content;
  }

  .message-group :deep(.left-text-message),
  .message-group :deep(.right-text-message) {
