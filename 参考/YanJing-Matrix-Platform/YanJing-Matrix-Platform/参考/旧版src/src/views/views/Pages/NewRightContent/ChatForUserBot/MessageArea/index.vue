...
// 获取bot用户ID
const getBotUserId = (): string => {
  try {
    const username = userInfoManager.getLoginField('username')
    if (!username) return ''
    const userId = removePrefixSuffix(username, '@', MATRIX_SERVER_URL_TAIL)
    return `@${userId}userbot${MATRIX_SERVER_URL_TAIL}`
  } catch (error) {
    console.error('[MessageArea] 获取bot用户ID失败:', error)
    return ''
  }
}
