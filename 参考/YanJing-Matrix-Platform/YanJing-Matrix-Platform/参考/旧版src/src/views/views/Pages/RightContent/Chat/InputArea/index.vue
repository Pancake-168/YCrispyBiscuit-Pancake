  ...
  <div v-if="attachments.length" class="attachments-preview" ref="attachmentsPreviewRef" @wheel.prevent="onPreviewWheel" @touchstart="onPreviewTouchStart" @touchmove="onPreviewTouchMove">
      <div v-for="(item, idx) in attachments" :key="item.id" class="attachment-chip">
        ...
        <div class="file-icon" v-else>
          <span>{{ item.file.type.split('/')[0].toUpperCase() || 'FILE' }}</span>
        </div>
        <div class="meta">
...
<rect width="40" height="40" rx="20" fill="#BAB3F1" fill-opacity="0.25"/>
<path d="M20 15.1665V25.8332" stroke="#404040" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M14.6665 20.4998H25.3332" stroke="#404040" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
    ...
    <div v-if="showMentionList" class="mention-list">
      ...
      >
        ...
        <span class="mention-user-id">{{ user.userId }}</span>
      </div>
    </div>
...
const searchUsers = async (searchText: string) => {
  ...
  try {
    ...
    const getMemberDisplayName = (member: any) =>
      resolveUserDisplayName({ matrixId: member.userId, matrixDisplayName: member.name })
    ...
    const filteredMembers = members
      .filter((member: any) => {
        ...
        return (
          displayName.toLowerCase().includes(loweredSearch) ||
          member.userId.toLowerCase().includes(loweredSearch)
...
const handleFileSelect = async (event: Event) => {
  ...
  if (!currentRoomId?.value) {
    console.error('没有当前房间ID，无法上传文件');
    // 这里可以添加用户提示，例如使用一个Toast通知
    return;
  }
...
.attachment-chip { display: inline-flex; align-items: center; gap: 8px; padding: 6px 8px; border: 1px solid var(--border-color); border-radius: 8px; background: var(--bg-color); }
...
.attachment-chip .file-icon { width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; border-radius: 4px; background: var(--bg-color-secondary); color: var(--text-color-secondary); font-size: 10px; }
.attachment-chip .meta { display: flex; flex-direction: column; max-width: 220px; }
.attachment-chip .meta .name { font-size: 12px; color: var(--text-color); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.attachment-chip .meta .size { font-size: 11px; color: var(--text-color-secondary); }
.attachment-chip .remove { margin-left: 4px; border: none; background: transparent; color: var(--text-color-secondary); cursor: pointer; font-size: 14px; }
.reply-target-banner { display: flex; justify-content: space-between; align-items: center; width: 100%; padding: 8px 12px; border-radius: 10px; background: var(--bg-color); border: 1px solid var(--border-color); gap: 8px; }
.reply-target-text { display: flex; flex-direction: column; gap: 2px; overflow: hidden; }
.reply-target-title { font-size: 12px; font-weight: 600; color: var(--text-color); white-space: nowrap; text-overflow: ellipsis; overflow: hidden; }
.reply-target-summary { font-size: 12px; color: var(--text-color-secondary); white-space: nowrap; text-overflow: ellipsis; overflow: hidden; }
.reply-target-close { border: none; background: transparent; color: var(--text-color-secondary); cursor: pointer; font-size: 16px; line-height: 1; padding: 2px 6px; }
...
.message-input {
  ...
  border-radius: 20px; /* 改为圆角 */
  padding: 6px 12px; /* 调整内边距，左侧增加一些 */
  align-items: center;
...
.message-input input {
  ...
  outline: none;
  padding-left: 4px; /* 减少一些，因为容器已经有了左侧padding */
}
...
.actions-menu {
  position: absolute;
  bottom: 110%; /* 从120%调整到110% */
  left: 0; /* 改为左对齐，与SVG左边缘对齐 */
  background-color: var(--bg-color-primary, var(--bg-color-secondary, #ffffff));
...
.send-button {
  padding: 4px 12px; /* 从6px 14px缩小到4px 12px */
...
.send-button:disabled {
  background-color: var(--bg-color-tertiary, #e9ecef);
  color: var(--text-color-secondary);
  cursor: not-allowed;
}
