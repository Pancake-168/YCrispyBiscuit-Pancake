...
import UserbotWsOptions from '@/components/Common/UserbotWsOptions'
import type { ResolveUserForOptionSend, UserbotWsOptionSendPayload } from '@/services/userbot/options'
import { extractUserbotOptions } from '@/services/userbot/options'
...
const wsUrl = computed(() => {
    const username = userBotStore.userBot?.username
    if (!username) return ''
    return `wss://t8962.zheshu.tech/api/v2/ws/shayu/${username}`
})
...
const getOrCreateEntry = (id: string): StreamEntry => {
    ...
    if (typeof idx === 'number') {
        return streamEntries.value[idx]
    }
...
const setChatInputText = inject<((text: string) => void) | null>('setChatInputText', null)
