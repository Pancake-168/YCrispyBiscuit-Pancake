...
// 转换数据为V2 包含部门和职位
function transformDataForECharts(node: OrgNodeV2, colors: any): any {
...
watch(() => props.rootNode, (newVal, oldVal) => {
  // 简单比较，避免不必要的重绘
  // 注意：V2 结构较深，JSON stringify 可能有性能损耗，但在视图层级尚可接受
  if (JSON.stringify(transformDataForECharts(newVal as OrgNodeV2, getNodeColors())) === 
      JSON.stringify(transformDataForECharts(oldVal as OrgNodeV2, getNodeColors()))) {
    return;
  }
  initChart();
}, { deep: true });
