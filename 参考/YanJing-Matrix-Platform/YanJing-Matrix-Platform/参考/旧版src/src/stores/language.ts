...
import { ref } from 'vue'
import en from '@/views/YanJingAI2/language/en-US'
import zh from '@/views/YanJingAI2/language/zh-CN'
...
export const useLanguageStore = defineStore('language', () => {
    ...
    async function setLanguage(lang: string): Promise<boolean> {
        ...
        console.log('[Language] 可以使用的语言列表:', list)
        if (!list.includes(lang)) {
            console.warn(`[系统语言] "${lang}" 没有注册。可用的语言有: ${list.join(', ')}`)
            return false
        }
        ...
        try {
            await localStorage.setItem('locale', lang)
        } catch {
    ...
    async function initLanguage() {
        try {
            const saved = await localStorage.getItem('locale')
