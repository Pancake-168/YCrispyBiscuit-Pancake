...
import type { OrgNodeV2, ApplicationV2 } from '@/types/organizationV2';
import type { GetApplicationUserResponse } from '@/types/application';
import { FetchAllDepartmentsV2, FetchDepartmentChildrenAllV2 } from '@/services/Project/OrganizationV2/department';
import { FetchPostUsersV2 } from '@/services/Project/OrganizationV2/Post';
import { ApplicationSync, GetApplicationUser } from '@/services/Project/OrganizationV2/Application';
import { buildV2Skeleton, mergeV2Children, mergePostUsers } from '@/services/Project/OrganizationV2/adapter';
import { userInfoManager } from '@/utils/userInfo';

export const useOrganizationStoreV2 = defineStore('organizationV2', () => {
    // --- State ---
     ...
     * 设置应用列表 (通常在登录或初始化时调用)
     */
    function setOrganizationList(list: any[]) {
        const v2List: ApplicationV2[] = list.map(item => ({
            app_id: String(item.app_id),
            app_tag: item.app_tag,
            name: item.name
        }));
        organizationList.value = v2List;
        console.log('[OrganizationStoreV2]组织列表已更新:', v2List.length);
    }
    ...
    function switchOrganization(org: ApplicationV2) {
        ...
        if (org.app_id) {
            ApplicationSync(String(org.app_id)).then(r => {
                if (!r.ok) {
                    console.warn('[OrganizationStoreV2] 应用同步失败 for', org.app_id);
                }
            }).catch(e => {
                console.warn('[OrganizationStoreV2] 应用同步异常 for ' + org.app_id, e);
            });
            ...
            loadApplicationUsers(appId)
                .catch(e => {
                    console.warn('[OrganizationStoreV2] 应用用户列表拉取异常 for ' + appId, e);
                })
                .finally(() => {
                    if (shouldLoadUsers && typeof window !== 'undefined') {
                        window.dispatchEvent(new CustomEvent('app:globalLoading', {
    ...
    async function loadSkeleton(appid: string, force = false) {
        if (!force && orgTrees.value.has(appid)) {
            ...
            if (tree && tree.length > 0) {
                ...
                if (listItem && !listItem.name) { // 仅当名称丢失时同步
                    ...
                    console.log(`[OrganizationStoreV2] 从现有骨架恢复名称: ${rootNode.name}`);
                }
                // 同步当前选中项
        ...
        try {
            ...
            if (res.ok && res.data?.data) {
                ...
                const rootNode = tree[0];
                if (rootNode) {
                    console.log(`[OrganizationStoreV2] 获取到根节点: ${rootNode.name} (${rootNode.id})`);
                    ...
                    if (listItem) {
                        listItem.name = rootNode.name;
                        console.log(`[OrganizationStoreV2] 已更新列表项名称: ${listItem.name}`);
                    }
                ...
                console.log(`[OrganizationStoreV2] 骨架加载完成 for ${appid}. Size: ${flatMap.size}`);
                ...
                if (deptNodes.length > 0) {
                    console.log(`[OrganizationStoreV2] 开始预加载 ${deptNodes.length} 个节点的职位信息...`);
                    ...
                    console.log(`[OrganizationStoreV2] 所有部门职位预加载完成`);
                }

            } else {
                console.warn(`[OrganizationStoreV2] 骨架加载失败或为空 for ${appid}`);
            }
        } catch (e) {
            console.error(`[OrganizationStoreV2] 骨架加载异常 for ${appid}`, e);
        } finally {
    ...
    async function loadNodeDetail(appid: string, nodeId: string) {
        ...
        if (!node) {
            console.warn(`[OrganizationStoreV2] 节点 ${nodeId} 没有在扁平索引中找到，无法加载详情`);
            return;
        }
        ...
        try {
            ...
            if (res.ok && targetData) {
                ...
                console.log(`[OrganizationStoreV2] 节点 ${nodeId} 详情加载完成. 子节点数量: ${node.children?.length}`);
            ...
            console.error(`[OrganizationStoreV2] 节点 ${nodeId} 详情加载异常`, e);
    ...
    async function loadPostDetail(appid: string, nodeId: string) {
        ...
        try {
            ...
            if (res.ok && Array.isArray(usersData)) {
                mergePostUsers(node, usersData, map); 
                ...
                console.log(`[OrganizationStoreV2] 职位 ${nodeId} 人员加载完成. 人员数量: ${node.children?.length}`);
            } else {
             ...
             console.error(`[OrganizationStoreV2] 职位 ${nodeId} 详情加载异常`, e);
             node.loadStatus = 'skeleton'; // 异常状态下重置，允许重试
        }
    }
    ...
    async function loadApplicationUsers(appid: string, force = false) {
        ...
        try {
            ...
            if (res.ok && res.data) {
                ...
                console.log('[OrganizationStoreV2] GetApplicationUser 加载完成 for', appid);
                console.log('[OrganizationStoreV2] 应用用户数量:', res.data.data.length);
                console.log('[OrganizationStoreV2] 应用用户:', res.data.data);
            } else {
                console.warn('[OrganizationStoreV2] GetApplicationUser 失败或为空 for', appid);
            }
        } catch (e) {
            console.warn('[OrganizationStoreV2] GetApplicationUser 异常 for ' + appid, e);
        }
    }
