...
import type { FetchPostUsersV2ApiResponse, FetchPostUsersV2Result } from "@/types/organizationV2";
...
export async function FetchPostUsersV2(appid: string, postId: number) {
    ...
    try {
        ...
        console.error('FetchPostUsersV2 报错:', error);
        return { ok: false, data: null } satisfies FetchPostUsersV2Result;
    }
}
...
export async function CreateOrganizationPostV2(appid: string, name: string, description: string, parentId: number) {
    ...
    try {
        ...
        console.error('CreateOrganizationPostV2 报错:', error);
        return { ok: false, data: null };
    }
}

// 更新组织 put请求
export async function UpdateOrganizationPostV2(appid: string, id: number, name: string, description: string, users: any[]) {
    const wechatStore = useWechatStore();
    const token = wechatStore.ssoParams.loginToken;
    ...
    console.log('UpdateOrganizationPostV2 参数:', appid, id, body);
    try {
        ...
        console.error('UpdateOrganizationPostV2 报错:', error);
        return { ok: false, data: null };
    }
}
...
export async function DeleteOrganizationPostV2(appid: string,id: number) {
    ...
    try {
        ...
        console.error('DeleteOrganizationPostV2 报错:', error);
        return { ok: false, data: null };
    }
}
