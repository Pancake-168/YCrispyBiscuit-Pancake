...
export default class Markdown {
            ...
            for (const { value } of foundLinks) {
              if (node?.firstChild?.literal) {
                ...
                if (newLinks.length === 1) {
                  ...
                  console.warn('[System:Markdown:repairLinks] Markdown links escaping found too many links for following text: ', text)
                  console.warn('[System:Markdown:repairLinks] Markdown links escaping found too many links for modified text: ', newText)
                }
              }
            }
          } else {
  ...
  public toHTML({ externalLinks = false } = {}): string {
    ...
    type TableNode = commonmark.Node & { _isTable?: boolean }
    type NodeRendererFn = (this: commonmark.HtmlRenderer, node: commonmark.Node, entering: boolean) => void
    type InlineRendererFn = (this: commonmark.HtmlRenderer, node: commonmark.Node) => void
    type VoidRendererFn = (this: commonmark.HtmlRenderer) => void
    let tableRenderActive = false

    // 辅助函数：检查节点是否在表格内
    const isInTable = (node: commonmark.Node): boolean => {
      let cur: commonmark.Node | null = node.parent
      while (cur) {
        if ((cur as TableNode)._isTable) return true
        cur = cur.parent
      }
      return false
    }

    const realParagraph = renderer.paragraph.bind(renderer) as NodeRendererFn
    renderer.paragraph = (function (this: commonmark.HtmlRenderer, node: commonmark.Node, entering: boolean) {
      // 尝试检测并渲染表格
      ...
      if (entering) {
               ...
               // 分隔符行的上一行是表头
                   ...
                   // 3. 渲染表格后的文本（如果有）
                     for (; i < lines.length; i++) {
                       const lineText = lines[i]
                       if (!lineText) continue
                       this.lit('<br />' + this.esc(lineText))
                   }
                   
                   // 标记该节点已作为表格渲染
                   ;(node as TableNode)._isTable = true
                   tableRenderActive = true
        ...
        if ((node as TableNode)._isTable) {
          tableRenderActive = false
          return
        }
      }
    ...
    const realText = renderer.text.bind(renderer) as InlineRendererFn
    renderer.text = (function (this: commonmark.HtmlRenderer, node: commonmark.Node) {
      if (isInTable(node)) return
      realText.call(this, node)
    }) as InlineRendererFn

    const originalSoftbreak = renderer.softbreak as unknown
    const rendererSoftbreak = renderer as unknown as { softbreak: VoidRendererFn }
    rendererSoftbreak.softbreak = function (this: commonmark.HtmlRenderer) {
      if (tableRenderActive) return
      if (typeof originalSoftbreak === 'function') {
        ;(originalSoftbreak as VoidRendererFn).call(this)
        return
      }
      this.lit(String(originalSoftbreak))
    }

    const originalLinebreak = renderer.linebreak as unknown
    const rendererLinebreak = renderer as unknown as { linebreak: VoidRendererFn }
    rendererLinebreak.linebreak = function (this: commonmark.HtmlRenderer) {
      if (tableRenderActive) return
      if (typeof originalLinebreak === 'function') {
        ;(originalLinebreak as VoidRendererFn).call(this)
      }
    }

    const realImage = renderer.image ? (renderer.image.bind(renderer) as NodeRendererFn) : undefined
    renderer.image = (function (this: commonmark.HtmlRenderer, node: commonmark.Node, entering: boolean) {
      if (isInTable(node)) return
      if (realImage) {
    ...
    const realEmph = renderer.emph.bind(renderer) as NodeRendererFn
    renderer.emph = (function (this: commonmark.HtmlRenderer, node: commonmark.Node, entering: boolean) {
      if (isInTable(node)) return
      realEmph.call(this, node, entering)
    }) as NodeRendererFn

    const realStrong = renderer.strong.bind(renderer) as NodeRendererFn
    renderer.strong = (function (this: commonmark.HtmlRenderer, node: commonmark.Node, entering: boolean) {
      if (isInTable(node)) return
      realStrong.call(this, node, entering)
    }) as NodeRendererFn

    const realCode = renderer.code.bind(renderer) as InlineRendererFn
    renderer.code = (function (this: commonmark.HtmlRenderer, node: commonmark.Node) {
      if (isInTable(node)) return
      realCode.call(this, node)
    }) as InlineRendererFn
