<template>
    <div class="matrix-register-form">
        <form @submit.prevent="handleSubmit" class="register-form">
            <h1 class="login-title">{{ bindTitle }}</h1>
            <div class="login-underline"></div>
            <p class="bind-hint">{{ bindHint }}</p>
            <div class="svg">
             <svg class="lingjingsvg" width="130" height="40" viewBox="0 0 527 174" fill="none"
                    ...
                    <path
                        ...
                        fill="url(#paint0_linear_wechat_bind_form)" />
                    <defs>
                        <linearGradient id="paint0_linear_wechat_bind_form" x1="6.21731e-10" y1="87" x2="664.38"
                            y2="87.2527" gradientUnits="userSpaceOnUse">
            ...
            <!--div class="form-group">
                ...
                <div class="input-icon-wrapper">
                    ...
                    <input id="reg-username" v-model="formData.username" :disabled="isRegistering" type="text"
                        placeholder="用户名" required class="form-input" />
                </div>
            </div>
            <small class="form-hint">*用户名只能包含: 小写字母、数字、下划线</small-->

            <div class="form-group">
                <label for="bind-phone"></label>
                <div class="input-icon-wrapper">
                    <svg class="input-icon" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                    ...
                    <input id="bind-phone" v-model="formData.phone" :disabled="isBinding" type="text"
                        placeholder="请输入手机号" required class="form-input" />
                </div>
            </div>

            <div v-if="isCaptchaStepVisible" class="form-group">
                <label for="wechat-bind-captcha"></label>
                <div class="captcha-row">
                    ...
                    <button type="button" class="refresh-captcha-button"
                        :disabled="isLoadingCaptcha || isBinding || isSendingCode" @click="refreshCaptcha">
                        换一张
                    </button>
                </div>
                <div class="input-icon-wrapper">
                    ...
                    <input id="wechat-bind-captcha" v-model="captcha.text" :disabled="isBinding || isSendingCode || isLoadingCaptcha"
                        type="text" placeholder="请输入图片验证码" required class="form-input" />
                </div>
            </div>

            <div class="form-group">
                <label for="bind-code"></label>
                <div class="verify-row">
                    <div class="input-icon-wrapper verify-input-wrapper">
                        <svg class="input-icon" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M7 8.5H17M7 12H14M9 3H15C18 3 20 5 20 8V16C20 19 18 21 15 21H9C6 21 4 19 4 16V8C4 5 6 3 9 3Z"
                                stroke="#C7C7C7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                        <input id="bind-code" v-model="formData.code" :disabled="isBinding" type="text"
                            placeholder="请输入短信验证码" required class="form-input" />
                    </div>
                    <button type="button" class="send-code-button"
                        :disabled="!isPhoneValid || isSendingCode || cooldownSeconds > 0 || isBinding"
            ...
            <!--div class="form-group">
                ...
                <div class="input-icon-wrapper">
                    ...
                    <input id="reg-confirm-password" v-model="formData.confirmPassword" :disabled="isRegistering"
                        type="password" placeholder="请再次输入密码" required class="form-input" />
                </div>
            </div>
            <small class="form-hint">*密码要求最短8位</small-->
            ...
            <button type="submit" :disabled="isBinding || !isFormValid" class="register-button">
                {{ isBinding ? '绑定中...' : '确认绑定并登录' }}
            </button>

            <div v-if="isBinding" class="loading-indicator">
...
type BindBranch = 'wechat' | 'register'
...
const emit = defineEmits<{
    'bind-success': [payload: { phone: string; code: string; branch: BindBranch }]
    'switch-to-login': []
}>()
...
const formData = reactive<BindFormData>({
    phone: '',
    code: ''
})

const bindTitle = computed(() => props.branch === 'wechat' ? '微信绑定手机号' : '绑定手机号')

const bindHint = computed(() => {
    if (props.branch === 'wechat') {
        return '您的微信已完成登录校验，请填写手机号并完成短信验证。'
    }
    return '请填写手机号并完成短信验证。'
})
...
const isFormValid = computed(() => {
    return isPhoneValid.value && /^\d{4,8}$/.test(formData.code)
})
...
const normalizeBindErrorMessage = (error: unknown): string => {
    const raw = error instanceof Error ? error.message : String(error)
    const detailMatch = raw.match(/"detail"\s*:\s*"([^"]+)"/)
    const detail = (detailMatch?.[1] || raw).toLowerCase()

    if (detail.includes('phone already registered')) return '该手机号已注册'
    if (detail.includes('phone already bound to another account')) return '该手机号已绑定到其他账户'
    ...
    if (detail.includes('captcha verification failed')) return '图片验证码错误或已过期，请重新输入'
    if (detail.includes('captcha verification required')) return '请先完成图片验证码校验'
    if (detail.includes('sms cooldown active') || detail.includes('429')) return '验证码发送过于频繁，请稍后再试'
    if (detail.includes('failed to update phone number')) return '手机号绑定失败，请稍后重试'
...
const loadCaptcha = async () => {
    ...
    try {
        ...
        captcha.id = ''
        captcha.image = ''
        errorMessage.value = normalizeBindErrorMessage(error)
    } finally {
...
const handleSendCode = async () => {
    ...
    try {
        const result = await wechatSSOService.sendBindCode(formData.phone, {
...
const handleSubmit = async () => {
    if (!isFormValid.value) {
        errorMessage.value = '请确保手机号和验证码填写正确'
        return
    }
...
const resetBindState = (message?: string) => {
...
defineExpose({
    resetBindState,
    resetRegisterState: resetBindState,
    resetLoginState: resetBindState,
})

console.log('[System:NewUserWithWechat:onMounted] NewUserWithWechat 组件已加载')
</script>
<style scoped>
