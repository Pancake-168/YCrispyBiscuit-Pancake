                ...
                <div class="tab-panel" v-if="activeTab === 'members'">
                    <div class="panel">
                        <div class="panel-header">
                            ...
                            <button v-if="canShowInviteButton" class="btn btn-primary"
                            ...
                            <div v-for="member in members" :key="member.userId" class="member-item">
                                <div class="member-info">
                                    ...
                                    <div class="member-meta">
                                        ...
                                        <div v-if="member.username" class="member-id">{{ member.username }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="member-actions" v-if="canManageMember(member)">
                                    <button
                                        v-if="canShowKickButton(member)"
                                        class="btn btn-ghost"
                                        :disabled="kickingUsername === member.username"
                                        @click="kickMember(member)">
                                        {{ kickingUsername === member.username ? '踢出中...' : '踢出' }}
                                    </button>
                                </div>
                            </div>
                        </div>
...
import { KickUsers, LeaveRoom as LeaveRoomRequest } from '@/services/Project/CreateRoom/Createroom'
...
const removeMemberFromList = (member: RoomMemberInfo) => {
    members.value = members.value.filter((item) => item.userId !== member.userId)

    const blobUrl = memberAvatarBlobMap.value[member.userId]
    if (blobUrl?.startsWith('blob:')) {
        URL.revokeObjectURL(blobUrl)
    }

    const nextMap = { ...memberAvatarBlobMap.value }
...
const mergeMemberProfiles = (
    ...
    return baseMembers.map((member) => {
        ...
        return {
            ...member,
            username: backendMember?.username || member.username,
...
const loadRoomData = async () => {
    ...
    isGroupRoom.value = await isGroupDisplayRoom(props.roomId)
...
const kickMember = async (member: RoomMemberInfo) => {
    if (!member.username) {
        openMessageDialog('该成员缺少 username，暂时无法踢出')
        return
    }

    const ok = await openConfirmDialog(`确定要将 ${member.displayName || member.username} 踢出当前群聊吗？`, {
        title: '确认踢出',
        confirmText: '踢出',
        cancelText: '取消',
    })
    ...
    kickingUsername.value = member.username
    try {
        const result = await KickUsers(props.roomId, [member.username])
        const kickedCount = Number(result.data?.kicked || 0)

        if (!result.ok || !result.data?.room_id || kickedCount <= 0) {
            throw new Error('接口调用失败')
        }

        removeMemberFromList(member)
        openMessageDialog(`已踢出 ${kickedCount} 名成员`)
        emit('roomUpdated', props.roomId)
    } catch (error: unknown) {
        openMessageDialog(`踢出失败: ${getErrorMessage(error)}`)
    } finally {
...
const leaveRoom = async () => {
    ...
    leaving.value = true
    try {
        const result = await LeaveRoomRequest(props.roomId)
        const leftCount = Number(result.data?.left || 0)

        if (!result.ok || !result.data?.room_id || leftCount <= 0) {
            throw new Error('接口调用失败')
        }
...
const canShowInviteButton = computed(() => {
    return permissions.value.isOwner && isGroupRoom.value
})

const canShowKickButton = (member: RoomMemberInfo) => {
    return (
        isGroupRoom.value
        && permissions.value.isOwner
...
const canManageMember = (member: RoomMemberInfo) => {
    return canShowKickButton(member)
}
