<template>
    <div class="system-map-wrapper" :class="{ 'system-map--mobile': isMobile }" @touchstart="handleTouchStart"
        ...
        <!-- 过滤条 -->
        <div class="system-map-filter" :style="filterStyle" v-show="showFilterBar">
        ...
        <template v-if="isMobile">
            <div class="system-map-track" :style="trackStyle">
                <div class="system-map-panel system-map-panel--left">
                    <VueFlow v-model:nodes="nodes" v-model:edges="edges" :node-types="nodeTypes"
                        ...
                        <Background variant="dots" :pattern-color="'color-mix(in srgb, var(--text-color) 18%, transparent)'"
                            :gap="40" :size="1.8" />
            ...
            <div class="mobile-panel-nav" role="tablist" aria-label="SystemMap mobile panel switch">
                <button
                    type="button"
                    class="mobile-panel-btn"
                    :class="{ 'is-active': mobilePanel === 'left' }"
                ...
                <button
                    type="button"
                    class="mobile-panel-btn"
                    :class="{ 'is-active': mobilePanel === 'right' }"
        ...
        <template v-else>
            <VueFlow v-model:nodes="nodes" v-model:edges="edges" :node-types="nodeTypes" :fit-view-on-init="true"
                ...
                @pane-click="() => handleNodeClickAndGoRight(null)">
                <!-- 背景网格：中间透明度和尺寸，平衡清晰度与精致感 -->
                <Background variant="dots" :pattern-color="'color-mix(in srgb, var(--text-color) 18%, transparent)'" :gap="40"
                    :size="1.8" />
...
import { MOBILE_MAX_WIDTH, toMaxWidthMediaQuery } from '@/constants/breakpoints';
...
const isMobile = ref(typeof window !== 'undefined' && window.matchMedia(toMaxWidthMediaQuery(MOBILE_MAX_WIDTH)).matches);
...
const goPrevPanel = () => {
    ...
    mobilePanel.value = panelOrder[nextIndex] ?? 'left';
};
...
.system-map--mobile .system-map-panel--right,
.system-map--mobile .system-map-panel--right .right-dock--mobile,
.system-map--mobile .system-map-panel--right .right-dock--mobile > * {
    min-width: 0;
    max-width: 100%;
    width: 100%;
...
.system-map--mobile .filter-row {
    gap: 8px;
}

.system-map--mobile .filter-types,
.system-map--mobile .filter-actions,
.system-map--mobile .filter-search,
.system-map--mobile .filter-count {
    width: 100%;
    min-width: 0;
}

.system-map--mobile .filter-btn {
    flex: 1;
}

.system-map--mobile .filter-input {
    min-width: 0;
}
...
.system-map--mobile .dock-resize-handle {
    display: none;
}

.system-map--mobile .internal-browser {
    max-width: 100%;
    min-width: 0;
}
